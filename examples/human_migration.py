...
df = pd.read_csv("data/state_pop_Annual.txt", delimiter="\t", parse_dates=["DATE"])
...
sorted_off_diagonal = np.sort((params.A - np.diag(np.diag(params.A))).flatten())
...
print("most noisy:", "From", columns[j], "to", columns[i], np.sqrt(V[i, j]) * 1e6)

V[V == 0] = 100000
i, j = np.where(V == np.min(V))
print("least noisy", "From", columns[j], "to", columns[i], np.sqrt(V[i, j]) * 1e6)
